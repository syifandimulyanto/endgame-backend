...
@section('content')
    @include('admin.layouts.partials.header', [
      'title' => (@$data ? 'Edit' : 'Add New') . ' Slider',
      'breadcrumbs' => [ ['Slider', route('admin.master.slider.index')], (@$data ? 'Edit' : 'Add New') . ' Slider']
    ])
    <div class="content">
        <div class="row">
                ...
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h5 class="panel-title">
                            <i class="icon-{{ (@$data ? 'pencil' : 'googleplus5') }}"></i> &nbsp; {{ (@$data ? 'Edit' : 'Add New') }} Slider
                        </h5>
                    ...
                    @if(@$data)
                        {{ Form::open(['route' => ['admin.master.slider.update', $data->id], 'method' => 'PUT', 'enctype' => 'multipart/form-data', 'class' => 'panel-body form-horizontal pb-20']) }}
                    @else
                        {{ Form::open(['route' => 'admin.master.slider.store', 'enctype' => 'multipart/form-data', 'class' => 'panel-body form-horizontal pb-20']) }}
                    @endif

                    <div class="form-group has-feedback {{ $errors->has('title') ? 'has-error' : '' }}">
                        <label class="control-label col-lg-4">Title <span class="text-danger">*</span></label>
                        <div class="col-lg-8">
                            <input type="text" class="form-control" name="title" placeholder="Title .." value="{{ @$data ? $data->title : old('title') }}" required>
                    ...
                    <div class="row pt-20">
                        <div class="col-xs-6">
                            <a href="{{ route('admin.master.slider.index') }}" class="btn btn-xs bg-slate-300 btn-labeled btn-ladda btn-ladda-spinner" data-style="slide-right">
